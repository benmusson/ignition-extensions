...
abstract class TagExtensions {
    ...
    @KeywordArgs(names = ["basePath", "recursive"], types = [String::class, Boolean::class])
    ...
    private fun TagConfigurationModel.toPyDictionary(): PyDictionary {
        return PyTagDictionary.Builder().setTagPath(path).setTagType(type).build(this).apply {
    ...
    private fun List<TagConfigurationModel>.toPyTagList() =
        fold(PyTagList()) { acc, childModel ->
            acc.add(childModel.toPyDictionary())
            acc
        }

    protected abstract fun getConfigurationImpl(
        basePath: TagPath,
        recursive: Boolean,
