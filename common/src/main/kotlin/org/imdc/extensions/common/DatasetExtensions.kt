...
object DatasetExtensions {
    ...
    fun map(args: Array<PyObject>, keywords: Array<String>): Dataset? {
        ...
        for (row in dataset.rowIndices) {
            val columnValues =
                Array<PyObject>(dataset.columnCount) { col -> Py.java2py(dataset[row, col]) }
    ...
    @Suppress("unused")
    @JythonElement(docBundlePrefix = "DatasetExtensions")
    @KeywordArgs(names = ["dataset", "filter"], types = [Dataset::class, PyFunction::class])
    fun filter(args: Array<PyObject>, keywords: Array<String>): Dataset? {
        ...
        val builder =
            DatasetBuilder.newBuilder().colNames(dataset.columnNames).colTypes(dataset.columnTypes)

        for (row in dataset.rowIndices) {
            val filterArgs =
                Array(dataset.columnCount + 1) { col ->
                    if (col == 0) {
                        Py.newInteger(row)
                    } else {
            ...
            if (returnValue) {
                val columnValues = Array(dataset.columnCount) { col -> dataset[row, col] }
    ...
    internal fun printDataset(
        ...
        val typeNames =
            List<String>(dataset.columnCount) { column ->
                if (includeTypes) {
        ...
        val columnWidths =
            IntArray(dataset.columnCount) { column ->
                maxOf(
                    ...
                    if (includeTypes) {
                        dataset.getColumnName(column).length +
                            typeNames[column].length +
                            3 // 3 = two parens and a space
                    } else {
                        dataset.getColumnName(column).length
                    },
    ...
    @Suppress("unused")
    @JythonElement(docBundlePrefix = "DatasetExtensions")
    @KeywordArgs(names = ["dataset", "filterNull"], types = [Dataset::class, Boolean::class])
    ...
    fun fromExcel(args: Array<PyObject>, keywords: Array<String>): Dataset {
        ...
        when (val input = parsedArgs.requirePyObject("input").toJava<Any>()) {
            ...
            is ByteArray -> WorkbookFactory.create(input.inputStream().buffered())
                ...
                throw Py.TypeError(
                    "Unable to create Workbook from input; should be string or binary data. Got ${input::class.simpleName} instead."
                )
        }.use { workbook ->
            ...
            val firstRow =
                parsedArgs.getInteger("firstRow").orElseGet {
                    max(sheet.firstRowNum, headerRow + 1)
                }
            ...
            val lastColumn =
                parsedArgs
                    .getInteger("lastColumn")
                    .map { it + 1 }
                    .orElseGet { columnRow.lastCellNum.toInt() }
            if (firstColumn >= lastColumn) {
                throw Py.ValueError(
                    "firstColumn ($firstColumn) must be less than lastColumn ($lastColumn)"
                )
            }
    ...
    @Suppress("unused")
    @JythonElement(docBundlePrefix = "DatasetExtensions")
    fun equals(@ScriptArg("dataset1") ds1: Dataset, @ScriptArg("dataset2") ds2: Dataset): Boolean {
    ...
    fun valuesEqual(
        ...
        return ds1.rowIndices.all { row ->
            ds1.columnIndices.all { col -> ds1[row, col] == ds2[row, col] }
        }
    }
    ...
    fun columnsEqual(
        ...
        val columnNamesMatch = columnNames.all { (left, right) -> left.equals(right, ignoreCase) }
        if (!columnNamesMatch) {
            return false
        }
        ...
        return columnTypes.all { (left, right) -> left == right }
    }

    @JythonElement(docBundlePrefix = "DatasetExtensions")
    @KeywordArgs(names = ["**columns"], types = [KeywordArgs::class])
    fun builder(args: Array<PyObject>, keywords: Array<String>): DatasetBuilder {
        if (args.size != keywords.size)
            throw Py.ValueError("builder must be called with only keyword arguments")
        val colNames = keywords.toList()
        val colTypes =
            args.mapIndexed { i, type ->
                try {
                    type.asJavaClass()
                } catch (e: ClassCastException) {
                    throw Py.TypeError(
                        "${keywords[i]} was a ${type::class.simpleName}, but should be a type or valid string typecode"
                    )
                }
            }
